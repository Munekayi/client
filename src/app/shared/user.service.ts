...
export class UserService {
  ...
  getCard(){
    ...
    return this.http.get('http://localhost:5000/api/card',{ headers });
  }
  ...
  getvoucherPayment(voucherNumber: voucherData){
    ...
    // Set the authorization header
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get(`http://localhost:5000/api/vouchers/${voucherNumber}`, { headers })
    .pipe(
      tap((voucher: any) => {
  ...
  addCard(newCard: cardData) {
    ...
    return this.http.post('http://localhost:5000/api/card',newCard,httpOptions)
  ...
  addUser(newUser: User)
  {
    ...
    return this.http.post('http://localhost:5000/api/user', newUser, httpOptions);
  }
  ...
  {
    return this.http.post('http://localhost:5000/api/authenticate', authCredentials);
  }
  ...
  getUserProfile() {
    ...
    // Set the authorization header
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get('http://localhost:5000/api/userprofile', { headers });
    ...
    getSensorData(meterId: string) {
      ...
      switchMap(() => this.http.get(`http://localhost:5000/api/sensordata/${meterId}`, { headers }))
      );
    }
    ...
    updateSensorData(meterId: string) {
     ...
     // const body = { amountowing: amount };
      return this.http.put(`http://localhost:5000/api/sensordata/${meterId}`,null, { headers });
    }
